...
/// TODO: Update summary.
/// </summary>
public class AsyncNetwork<T> : IDisposable
    where T : IFloatingPoint<T>
{
    private NetworkPipeline<T> _pipeline;
    private IPropagatorBlock<LayerData<T>, LayerData<T>> _networkPipeline;
    private ActionBlock<LayerData<T>> _resultSink;
    private IDisposable _pipelineToSink;
    private ObjectPool<LayerData<T>> _layerDataPool;
    ...
    {
        _pipeline = new NetworkPipeline<T>(network, maxDegreeOfParallelismPerLayer);
        _networkPipeline = _pipeline.Transformer;
        _resultSink = new ActionBlock<LayerData<T>>(
            (result) =>
            {
                var tcs =
                    (TaskCompletionSource<Vector<T>>)result.State;
        ...
        _layerDataPool = ObjectPool.Create<LayerData<T>>();
    }
    ...
    /// <returns></returns>
    public Task<Vector<T>> ComputeAsync(Vector<T> input)
    {
        ThrowIfDisposed();
